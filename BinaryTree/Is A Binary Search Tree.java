...
public class Main {
  ...
  public static class pair{
    ...
    int max;
    int min;
    public pair(boolean isBst,int max,int min)
    {
      this.isBst=isBst;
      this.max=max;
      this.min=min;
...
if(root==null)
{
  pair bp=new pair(true,Integer.MIN_VALUE,Integer.MAX_VALUE);
  return bp;
}
...
pair mypair=new pair(true,max,min);
return mypair;
}

 else{
   pair ne=new pair(false,0,0);
   return ne;
 }
